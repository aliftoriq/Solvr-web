...
export class AuthService {
  ...
  login(username: string, password: string): Observable<any> {
    return this.http
      .post<any>(this.authUrl + '/login-employee', {
        username,
        password,
      })
      .pipe(
        tap((res) => {
          if (this.isBrowser && res.data?.token) {
            ...
            if (res.data.user?.name)
              localStorage.setItem(this.name, res.data.user.name);
            if (res.data.user?.role)
              localStorage.setItem(this.role, res.data.user.role);
            if (res.data.features)
              localStorage.setItem(this.feature, res.data.features);
          }
        })
      );
  }
  ...
  forgetPassword(username: string): Observable<any> {
    return this.http.post<any>(this.authUrl + '/reset-password', { username });
  }
